...
public class MapView extends View {
	...
	protected final Point offsetPoint;
	...
	public static Paint MapPaint = new Paint();
	public static Paint RedPaint = new Paint();
			

	public MapView (OpenStreetMapViewerActivity activity, int mscreenWidth, int mscreenHeight, int cacheSize, String dbPath, Resources res) {
		super(activity);	
    	...
    	tileManager = new TileManager(this, cacheSize, dbPath, res);
	...
	public void drawMap(Canvas canvas) {		
		...
		final Tile tile = new Tile(0,0,zoom);
		Bitmap bitmap;
		Point userPositionPoint;
		final int PivotMinusOffsetX = -pivotPoint.x - offsetPoint.x;
		final int PivotMinusOffsetY = -pivotPoint.y - offsetPoint.y;
		final int FOUR = 4;
		...
		for(int x = firstX , i = 0 ; i < maxXTiles ; x++, i++ ) {
			for(int y = firstY, j = 0 ; j < maxYTiles; y++ , j++) {
				...
				tile.setX(x);
				tile.setY(y);
				bitmap = tileManager.getTile(tile);	
				currentTiles.add(tile);
				if(bitmap != null) {
					if(calculateRatio==true)
						cacheHits++;
					canvas.drawBitmap(bitmap, 
									  PivotMinusOffsetX + i*TILE_SIZE, 
									  PivotMinusOffsetY + j*TILE_SIZE, 
									  MapPaint); 
					
					if(Tile.equalsTile(tile,realPivotTile)){
							
						userPositionPoint = getPointInTile(latitude, longitude, zoom);
						canvas.drawRect(PivotMinusOffsetX + userPositionPoint.x + i*TILE_SIZE - FOUR, 
								        PivotMinusOffsetY + userPositionPoint.y + j*TILE_SIZE - FOUR,								
										PivotMinusOffsetX + userPositionPoint.x + i*TILE_SIZE + FOUR, 
										PivotMinusOffsetY + userPositionPoint.y + j*TILE_SIZE + FOUR, 
										RedPaint);
					}
