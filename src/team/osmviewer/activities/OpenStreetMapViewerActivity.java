...
import team.osmviewer.util.NetHandler;
...
public class OpenStreetMapViewerActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        dbPath = getBaseContext().getFilesDir().getPath() + "/" + DB_NAME;
        
        view = new MapView(this, display.getWidth(), display.getHeight(), CACHE_SIZE, dbPath, res);        
...
/*        returnOnFollowMe = new Button(this);
        ...
        setContentView(R.layout.main);
       	LinearLayout rel = (LinearLayout)findViewById(R.id.mapViewContainer);
       	LinearLayout.LayoutParams viewParams = new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT);        
        rel.addView(view, viewParams);
        
        btnSend = (Button)findViewById(R.id.btn_send);
        btnSend.setOnClickListener(SendCommandListener);
        cmdLine = (EditText)findViewById(R.id.ed_commandline);
        
        netHandler = new NetHandler();

    }
    
    private OnClickListener SendCommandListener = new OnClickListener() {
        public void onClick(View v) {        
        	String command = cmdLine.getText().toString();
        	if (command.length() != 0){
